...
- Real reply runners that share the same live SQLite file atomically reserve the normalized target before browser staging and finalize durable history only after the native staging flow returns. `--force` may intentionally bypass finalized history, but it never bypasses an active, stale, or ambiguous reservation. Separate database files are not coordinated.
- An interrupted or ambiguous staging attempt retains its reservation. A 24-hour-old claim is only eligible for explicit recovery; age never deletes it or starts staging. First ensure the prior process stopped and check X Unsent/Drafts in the exact CLI-owned profile used by that run. If a matching draft exists or the comparison is uncertain, leave the reservation in place and do not retry or use `--force`. Only after confirming no matching reply draft exists, `--recover-stale-reservation-after-confirming-no-draft` clears the stale claim and exits without staging. That recovery flag cannot be combined with content, `--force`, `--dry-run`, `--inspect`, or `--long`.
- A staging-runtime initialization failure proves that the browser staging function was never invoked, so only that owner-matched reservation may be released. Once `stageReplyDraft` is invoked, a thrown or unusable result retains the claim because the current poster exposes no pre-Save phase evidence; selector calibration and `--inspect` cannot bypass or clear it.
- If native reply staging returns but reply-ledger finalization or close fails, the command exits nonzero because the draft may already exist while durable deduplication is missing or uncertain. Never retry automatically: before any retry, compare X Unsent/Drafts manually in the exact CLI-owned profile used by that run. Selector calibration and `--inspect` do not repair a ledger failure.
