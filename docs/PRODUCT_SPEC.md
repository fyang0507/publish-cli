...
| **微信公众号 (WeChat Official Account)** | Programmable (API) — **PUBLISH built + live-verified 2026-07-04** | Limited (out of scope) | The **first API-driven** channel: `api.weixin.qq.com`, app_id/app_secret → stable-token, native `draft/add`. The **never-publish boundary is structural** — save (`draft/add`) and publish are separate endpoints, so the tool only ever calls draft. Requests route through a **fixed egress IP** (allowlisted once) to satisfy WeChat's IP白名单 gate zero-touch. WATCH is out of scope (weak on-platform discovery). See `docs/WECHAT_DESIGN.md`. |
...
| **Phase 3 — 小红书 + WeChat** | Add 小红书 (Playwright persistent profile) and 微信公众号 (API publish). **WeChat PUBLISH (`check` / `draft`) landed — built + live-verified 2026-07-04 (first API-driven channel; native `draft/add`, fixed-egress-IP allowlist); 小红书 remains.** | Full 5-channel fan-out | None — native drafts only |
